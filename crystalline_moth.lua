...
-- Old killer aimbot removed; Enstrangle lock-on (below) replaces it.
-- aimAmIKiller kept as a shared utility for other systems that reference it.
...
-- aimUnlock stub so applyConfigFromFlags doesn't error
local function aimUnlock() end
...
-- ============================================================
-- ENSTRANGLE TRUE LOCK-ON (Grab Hook Aimbot)
-- Integrated into BetrayalHub killer logic
-- Triggers on UseActorAbility via hookmetamethod; slash/parry unlocks.
-- ============================================================

local secEnstrangle = tabKiller:Section({ Title = "Enstrangle Lock-On", Opened = false })

local enst = {
    on            = false,
    slashUnlocks  = true,     -- if false, only unlocks on survivor death
    maxDist       = 80,
    predMult      = 0.12,     -- base prediction multiplier
    locked        = false,
    target        = nil,
    lockConn      = nil,
    lastAbility   = "",
...
-- Find nearest survivor HRP within enst.maxDist
local function enstNearest()
    ...
    for _, model in ipairs(survivors:GetChildren()) do
        if model ~= ch and model:IsA("Model") then
            local r = model:FindFirstChild("HumanoidRootPart")
            local h = model:FindFirstChildOfClass("Humanoid")
            if r and h and h.Health > 0 then
                local d = (r.Position - hrp.Position).Magnitude
                if d < bestDist and d <= enst.maxDist then bestDist = d; best = r end
            end
        end
    end
    return best
...
-- Predict target head position with velocity compensation
local function enstHeadPos(targetHRP)
    ...
    if targetHRP.AssemblyLinearVelocity then
        ...
        local mult  = enst.predMult
        if speed > 55 then mult = 0.35
        elseif speed > 30 then mult = 0.22 end
        pos = pos + vel * mult
    end
    return pos
end

-- Camera + body rotation true lock-on (same as Enstrangle standalone)
local function enstDoLock(survivorHeadPos)
    local ch  = lp.Character; if not ch then return end
    local hrp = ch:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    local cam = svc.WS.CurrentCamera; if not cam then return end
    if cam.CameraType ~= Enum.CameraType.Custom then cam.CameraType = Enum.CameraType.Custom end

    local dist = (hrp.Position - survivorHeadPos).Magnitude
    if dist > 0.01 then
        local dirFromSurv  = (hrp.Position - survivorHeadPos).Unit
        local camOffset    = dirFromSurv * 5
        local targetCamPos = hrp.Position + camOffset + Vector3.new(0, 3, 0)
        local lerpSpd      = dist > 40 and 0.9 or 0.5
        cam.CFrame = cam.CFrame:Lerp(CFrame.new(targetCamPos, survivorHeadPos), lerpSpd)
    end

    pcall(function()
        local lookDir = Vector3.new(survivorHeadPos.X - hrp.Position.X, 0, survivorHeadPos.Z - hrp.Position.Z)
        if lookDir.Magnitude > 0.01 then
            local tgt = CFrame.new(hrp.Position, hrp.Position + lookDir.Unit)
            hrp.CFrame = hrp.CFrame:Lerp(tgt, 0.6)
        end
    end)
    ...
    if mousemoverel then
        pcall(function()
            ...
            if onScreen then
                ...
                mousemoverel((screenPos.X - vs.X / 2) * 0.6, (screenPos.Y - vs.Y / 2) * 0.6)
            end
        end)
    end
end

local function enstUnlock()
    if enst.lockConn then enst.lockConn:Disconnect(); enst.lockConn = nil end
    enst.locked = false; enst.target = nil
    pcall(function()
        local cam = svc.WS.CurrentCamera
...
local function enstStartLock(targetHRP)
    enstUnlock()
    if not targetHRP or not targetHRP.Parent then return end
    enst.target = targetHRP; enst.locked = true
    enst.lockConn = svc.Run.RenderStepped:Connect(function()
        if not enst.locked or not enst.target or not enst.target.Parent then enstUnlock(); return end
        local hum = enst.target.Parent:FindFirstChildOfClass("Humanoid")
        if not hum or hum.Health <= 0 then enstUnlock(); return end
        local pos = enstHeadPos(enst.target); if not pos then return end
        enstDoLock(pos)
    end)
end

-- Decode ability name from buffer/string arg
local function enstGetAbilityName(args)
    local data = args[2] and args[2][1]; if not data then return "" end
    local name = ""
    pcall(function()
...
-- Hook UseActorAbility FireServer — only active when enst.on and player is killer
-- Reuses BetrayalHub's existing aimAmIKiller() function
if typeof(hookmetamethod) == "function" then
    local _enstOldNC
    _enstOldNC = hookmetamethod(game, "__namecall", function(self, ...)
        local method = getnamecallmethod()
        if method == "FireServer" then
            local args = {...}
            if args[1] == "UseActorAbility" and enst.on and aimAmIKiller() then
                task.spawn(function()
                    local abilityName = enstGetAbilityName(args)
                    enst.lastAbility = abilityName
                    -- Slash/parry = unlock only if slashUnlocks toggle is on
                    if enst.slashUnlocks and (abilityName:lower():find("slash") or abilityName:lower():find("parry")) then
                        enstUnlock()
                    else
                        local t = enstNearest()
                        if t then enstStartLock(t) end
                    end
                end)
            end
        end
        return _enstOldNC(self, ...)
    end)
else
    -- Fallback: piggyback on the existing remote.OnClientEvent trigger
    task.spawn(function()
        pcall(function()
            local remote = hbGetRemote(); if not remote then return end
            remote.OnClientEvent:Connect(function(...)
                if not enst.on or not aimAmIKiller() then return end
                local a={...}; if typeof(a[1])~="string" then return end
                local n = a[1]
                if n:match("Ability") or n:match("[QER]") or n=="Slash" or n=="Dagger" then
                    if enst.slashUnlocks and n:lower():find("slash") then
                        enstUnlock()
                    else
                        local t = enstNearest(); if t then enstStartLock(t) end
                    end
                end
            end)
        end)
...
-- Unlock on respawn
lp.CharacterAdded:Connect(function() enstUnlock() end)

-- UI controls
secEnstrangle:Toggle({
    Title = "Enable Enstrangle Lock-On",
    Type  = "Checkbox",
    Flag  = "enstOn",
    Default = enst.on,
    Callback = function(on)
...
secEnstrangle:Toggle({
    Title = "Slash Unlocks",
    Desc  = "OFF = only unlocks when survivor dies",
    Type  = "Checkbox",
    Flag  = "enstSlashUnlocks",
    Default = enst.slashUnlocks,
    Callback = function(on)
        pcall(function() enst.slashUnlocks = on end)
    end
})

secEnstrangle:Slider({
    Title = "Max Distance",
    Flag  = "enstMaxDist",
    Step  = 5,
    Value = { Min = 10, Max = 200, Default = enst.maxDist },
    Callback = function(v) pcall(function() enst.maxDist = v end) end
})
...
secEnstrangle:Slider({
    ...
    Flag  = "enstPredMult",
    Step  = 0.01,
    Value = { Min = 0.0, Max = 0.5, Default = enst.predMult },
    Callback = function(v) pcall(function() enst.predMult = v end) end
})

secEnstrangle:Button({
    Title = "Manual Unlock",
    Callback = function() enstUnlock() end
})
...
local function gvStartStandalone()
    ...
    gvStandaloneThread = task.spawn(function()
        ...
        -- Interval-based loop so scanRate slider controls actual frequency
        task.spawn(function()
            while gvEsp.on do
                task.wait(math.max(0.1, gvConfig.ScanInterval))
                if not gvEsp.on then break end
                updateLoop()
                -- Live-propagate opacity/colour to existing highlights
                for model, data in pairs(trackedEntities) do
                    pcall(function()
                        if data.Highlight and data.Highlight.Parent then
                            data.Highlight.FillTransparency    = gvConfig.FillTransparency
                            data.Highlight.OutlineTransparency = gvConfig.OutlineTransparency
                            data.Highlight.FillColor           = gvConfig.HighlightColor
                            data.Highlight.OutlineColor        = gvConfig.HighlightColor
                        end
                    end)
                end
            end
...
-- Live helper: push gvConfig values to all currently tracked highlights
local function gvPropagateConfig()
    for _, obj in ipairs(gvStandaloneObjects) do
        local _, hl = obj[1], obj[2]
        pcall(function()
            if hl and hl.Parent then
                hl.FillTransparency    = gvConfig.FillTransparency
                hl.OutlineTransparency = gvConfig.OutlineTransparency
                hl.FillColor           = gvConfig.HighlightColor
                hl.OutlineColor        = gvConfig.HighlightColor
            end
        end)
    end
end
...
secGVEsp:Slider({
    Title = "Fill Opacity (transparency)",
    Flag = "gvFillTransparency",
    Step = 0.05,
    Value = { Min = 0.0, Max = 1.0, Default = gvConfig.FillTransparency },
    Callback = function(v)
...
secGVEsp:Slider({
    Title = "Outline Opacity (transparency)",
    Flag = "gvOutlineTransparency",
    Step = 0.05,
    Value = { Min = 0.0, Max = 1.0, Default = gvConfig.OutlineTransparency },
    Callback = function(v)
        gvConfig.OutlineTransparency = v
        gvPropagateConfig()
    end
})

secGVEsp:Slider({
    Title = "Scan Rate (interval s)",
    Flag = "gvScanInterval",
    Step = 0.05,
    Value = { Min = 0.1, Max = 3.0, Default = gvConfig.ScanInterval },
    Callback = function(v)
        -- Just update gvConfig; the loop reads it each cycle — no restart needed
        gvConfig.ScanInterval = v
    end
})
...
function applyConfigFromFlags()
    pcall(function()
        ...
        -- (old aim.* flags removed; Enstrangle aimbot handles this now)
        ...
        if type(win.Flags.gvFillTransparency)=="number" then gvConfig.FillTransparency=win.Flags.gvFillTransparency; pcall(gvPropagateConfig) end
        if type(win.Flags.gvOutlineTransparency)=="number" then gvConfig.OutlineTransparency=win.Flags.gvOutlineTransparency; pcall(gvPropagateConfig) end
        if type(win.Flags.gvScanInterval)=="number" then gvConfig.ScanInterval=win.Flags.gvScanInterval end
        if win.Flags.enstOn ~= nil then enst.on=win.Flags.enstOn; if not enst.on then pcall(enstUnlock) end end
        if win.Flags.enstSlashUnlocks ~= nil then enst.slashUnlocks=win.Flags.enstSlashUnlocks end
        if type(win.Flags.enstMaxDist)=="number" then enst.maxDist=win.Flags.enstMaxDist end
        if type(win.Flags.enstPredMult)=="number" then enst.predMult=win.Flags.enstPredMult end
