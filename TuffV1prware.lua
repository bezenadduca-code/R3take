...
local function createEmoteGui()
    ...
    -- ============================================================
    -- STOP / PLAY (True Global Audio & Attached Effects)
    -- ============================================================
    local function StopEmoting()
        ...
        -- Clean up true global sound fallback
    ...
    local function PlayEmote(mod)
        ...
        if data.SFX then
            ...
            -- 1. Sound on humanoid (for directional immersion)
            if hum then
            ...
            -- 2. True global sound (placed in SoundService so everyone hears it globally regardless of distance)
        ...
        -- 3. Attach Effects directly to character Root (spawning real game effects like RootAttachment & bursts)
        ...
        -- Hakari Decompiled Server/Visual Execution integrated locally for Emotes
        ...
        -- Fire server remote for game replication (if available)
    ...
    -- ============================================================
    -- TOGGLE BUTTON  — right down corner
    -- ============================================================
    ...
    -- Scroll list — leaves 36px at bottom for stop button
...
local function shiftlockFollowCamera()
    ...
    -- Check if shiftlock is active via UserInputService
    local shiftLock = svc.Input:GetPropertyChangedSignal("MouseBehavior")
    -- We'll use a different approach - check if MouseBehavior is LockCenter
    
    local cam = svc.WS.CurrentCamera
    if not cam then return end
    
    -- Get the camera's look direction (horizontal only)
    ...
    -- Only apply when shiftlock is on (MouseBehavior == LockCenter)
